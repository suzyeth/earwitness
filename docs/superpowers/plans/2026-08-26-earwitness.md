...
├── demo-probe-01.ts                          runs the real fixture end to end, no network
├── fixtures/
...
- [ ] **Step 4b: Make the safety gates observable**

`main` takes an optional `MainDeps` so tests can inject a client and a judge. Real usage passes
nothing and behaves identically. Without this, two guarantees have no coverage and could be
deleted with nothing going red: that `diff` works with no credentials, and that a malformed
policy aborts before a call is billed.

Create `src/cli/main.test.ts` with three tests: `diff` succeeding with both API keys deleted
from `process.env`; a policy naming `grounded` without a `field` rejecting while a client double
whose `placeCall` throws is never reached; and a sound policy actually reaching that client once,
so deleting the scenario loop cannot satisfy the previous test.
